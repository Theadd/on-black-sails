...
module.exports = {
  ...
  /** DEPRECATED
   * HashController.raw_search()
   * @example: http://localhost:1337/hash/raw_search?query=2014%20720%20|%201080&category=movie%20|%20tv
   */
  raw_search: function (req, res) {
    var i,
      query = Hash.find(),
    ...
    query.exec(function searchCB(err, hashes){
      ...
      if (typeof req.param('define') !== "undefined" && req.param('define').length) {
        hashes = applyResultsModifier(hashes, req.param('define'))
...
function applyResultsModifier (hashes, modifier) {
